...
class UserNewsController extends GetxController {
  ...
  void updateDateMap() {
    ...
    for (var e in userNewsList) {
      ...
      String date = '';
      if (e.date!.day.isEqual(DateTime.now().day) &&
          e.date!.month.isEqual(DateTime.now().month)) {
        date = AppLocalizations.of(Get.context!)!.aujourdhui;
      }
      //check if it is yesterday
      else if (e.date!.day.isEqual(DateTime.now().day - 1) &&
              e.date!.month.isEqual(DateTime.now().month) ||
          (DateTime.now().day == 1) &&
              (e.date!.month.isEqual(DateTime.now().month - 1))) {
        date = AppLocalizations.of(Get.context!)!.hier;
      } else {
        String dayOfWeek =
            DateFormat.EEEE(Get.locale!.languageCode).format(e.date!);
        String dayMonth =
            DateFormat.MMMMd(Get.locale!.languageCode).format(e.date!);
