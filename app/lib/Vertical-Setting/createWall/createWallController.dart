...
class VSCreateWallController extends GetxController {
  ...
  void onTapPlus() {
    ...
    List<TextEditingController> ouvreurNamesNew = createWallModules.isNotEmpty
        ? createWallModules.last.ouvreurNames
    ...
    createWallModules.add(CreateWallWidget(
        ...
        attributesAllList: climbingLocationResp!.attributes,
  ...
  Future<void> createWallConfirm(BuildContext context) async {
    try {
      Get.find<VSHomeController>().creationImage.value = image[0];
      Get.back();
      ...
      await Get.find<VSHomeController>().getGym();
      ...
      ProgressDialogUtils.hideProgressDialog();
      // if (e.error!.keys.contains('name')) nameError = e.error!['name'][0];
